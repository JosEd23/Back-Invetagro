...
function registrar(req,res){
    ...
    venta.pago = data.pago;
    venta.comprobante = data.comprobante;
    venta.num_comprobante = data.num_comprobante;
...
function listado_pedido(req,res){
    ...
    Venta.find({"comprobante" : "ENVIO"}).populate('idcliente').populate('iduser').exec((err,data_ventas)=>{
        if(data_ventas){
            res.status(200).send({ventas:data_ventas});
        }else{
            res.status(404).send({message: "No hay ningun registro de venta"});
